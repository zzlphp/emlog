 ...
 * 分页函数
 */		
function newpagenav($count, $perlogs, $page, $url, $anchor='') {
	$pnums = @ceil($count / $perlogs);
	$re = '';
	if($page!==1){
		$prepage = $page-1;
		$re .= "<a class=\"new-posts\" href=\"$url$prepage$anchor\">← Newer Posts</a>";
	}
	$re .= " <span> Page ".$page." of ".$pnums." </span> ";
	if($page!=$pnums){
		$nextpage = $page+1;
		$re .= "<a class=\"older-posts\" href=\"$url$nextpage$anchor\">Older Posts →</a>";
	}
	return $re;
}

//获取摘要的第一张大图
function getBigImg($gid){
	$db=MySql::getInstance();
	$sql = "SELECT excerpt FROM ".DB_PREFIX."blog WHERE checked='y' and password='' and gid=".$gid;
	$query = $db->query($sql);
	while ($row = $db->fetch_array($query)) {
		preg_match_all("/<img([^>]+)src=\"([^>\"]+)\"?([^>]*)>/i",$row['excerpt'],$matches);//从摘要找缩略图
		if($matches[2][0]){
...
function blog_comments($comments){
	...
	<div class="comment" id="comment-<?php echo $comment['cid']; ?>">
		...
		<div class="comment-info">
			<b class="comment-info-name"><?php echo $comment['poster']; ?> </b><br /><span class="comment-time comment-reply2"><?php echo $comment['date']; ?>&nbsp;&nbsp;<a href="#comment-<?php echo $comment['cid']; ?>" onclick="commentReply(<?php echo $comment['cid']; ?>,this)">回复</a></span>
